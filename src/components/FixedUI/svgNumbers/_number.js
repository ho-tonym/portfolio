...
const Number = ({ color }) => {
  ...
  useEffect(() => {
    toggleNumber(false)//finishes closing anim in 800
    setTimeout(() => setSvgValues({
      stroke: svgData[currentProj].stroke,
      d: svgData[currentProj].d,
    }), 800)
    // setTimeout(() => setSvgValues({
    //   stroke: svgData[currentProj].stroke,
    //   d: svgData[currentProj].d,
    // }), 900)// when thats done we change it
    setTimeout(() => toggleNumber(true), 1000) // a bit later than 800 we open it up
  }, [currentProj])
